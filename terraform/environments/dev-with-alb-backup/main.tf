...
# Data: Get current AWS account info
...
# NAT Gateway (only 1 for dev to save cost)
...
# ECR Lifecycle Policy - Keep only last 10 images to save cost
resource "aws_ecr_lifecycle_policy" "app" {
  ...
  policy = jsonencode({
    rules = [
      {
        rulePriority = 1
        description  = "Keep last 3 images (dev environment)"
        selection = {
          tagStatus   = "any"
...
resource "aws_cloudwatch_log_group" "app" {
  ...
  retention_in_days = 7 # Keep logs for 7 days (dev environment)
...
resource "aws_iam_role_policy" "ecs_task_execution_ecr" {
  name_prefix = "${local.name_prefix}-ecr-"
